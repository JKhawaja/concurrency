...
Well, in our previous mentioned example, if that VDG that applied to two UIs lasted the lifespan of the CDS then it would simply be acting as a spatial dependency "representation" (since it would signify that all writes for the dependent UI -- that is attached to -- must wait on all writes for the dependency UI that it is also attached to). 

And ff the VDG had only applied to one of the UIs then it would have represented a permanent temporal DAG for similar reasons. The only thing that makes a VDG dynamic is the fact that it's lifespan is always less then that of the CDS.
...
The final dynamic property of a system we want to briefly discuss is the idea of: dynamic UIs.
...
Because a Virtual UI is not dependent on the same premises that real UIs are dependent on, we provide the following lemma to show an important difference between virtual and real UIs.
