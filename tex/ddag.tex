...
\begin{con-ex} [2-Regular Tree]
	...
	We consider boundary nodes of a UI dependency graph for a 2-Regular Tree CDS. We will consider a 3 node 2-Regular Tree for which we assign one node to a UI and make the root node's UI dependent on the the two leaf nodes UIs. This makes our UI DDAG have one root-boundary and two leaf-boundary nodes. In this case, we will allow structure appending access types on any three of our nodes.
